...
# Load the custom indicators so they register themselves
from . import smart_money_concepts
from . import trend_oscillators

# Keep the base indicators from indicators_old to supply ATR, Swing High, SMA, etc.
...
def add_indicators(df: pd.DataFrame) -> pd.DataFrame:
    """Calculates Base Indicators + New Custom 126-Engine Indicators"""
    enriched = df.copy()
    
    # 1. Base Core calculations needed by advanced indicators
    ...
    # 2. Run the dynamic registry indicators
    enriched = registry.apply_all(enriched)
