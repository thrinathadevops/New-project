...
def fetch_yahoo_ohlcv(ticker: str, period: str = "60d", interval: str = "5m") -> pd.DataFrame:
    import yfinance as yf

    data = yf.download(
        normalize_ticker_for_yahoo(ticker),
        period=period,
        interval=interval,
        auto_adjust=True,
        progress=False,
...
def generate_sample_ohlcv(rows: int = 260, seed: int = 7) -> pd.DataFrame:
    rng = np.random.default_rng(seed)
    index = pd.date_range("2025-01-01 09:15", periods=rows, freq="15min")
    returns = rng.normal(0.00035, 0.006, size=rows)
    close = 250 * np.exp(np.cumsum(returns))
    spread = rng.uniform(0.002, 0.018, size=rows) * close
